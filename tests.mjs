...
const errorPage=fs.readFileSync('dist/404.html','utf8');
if(fs.existsSync('dist/404'))throw new Error('Error document must not create a normal /404/ page');
if(!errorPage.includes('<meta name="robots" content="noindex">')||/rel="canonical"/.test(errorPage)||!errorPage.includes('<h1>That page is not here.</h1>'))throw new Error('Custom 404 indexability or content regressed');
...
for(const file of fs.readdirSync('dist',{recursive:true}).filter(f=>f.endsWith('.html')&&f!==verification)){
 ...
 if((route==='404.html'?canonical!==undefined:canonical!==expected)||og!==expected||data?.url!==expected)throw new Error(`SEO mismatch: ${file}`);
...
if(sitemapUrls.some(url=>/\/404(?:\/|\.html)?$/.test(url)))throw new Error('Error document must not appear in sitemap');
